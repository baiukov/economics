...
export class Task2112 implements ITask {
	private taskNumber = 2112;
	...
	public constructor() {
		...
		this.task = `
			...
			Vypočítejte meziroční změny nominálního HDP a reálního HDP.
		`
	...
	private createAnswerHTML = () => {
		...
		const inputYNom2016 = document.createElement('input')
		inputYNom2016.setAttribute("type", "text")
		inputYNom2016.setAttribute("id", `task-${this.taskNumber}-answer-yNom2016`)
		const inputYNom2017 = document.createElement('input')
		inputYNom2017.setAttribute("type", "text")
		inputYNom2017.setAttribute("id", `task-${this.taskNumber}-answer-yNom2017`)

		const inputYReal2016 = document.createElement('input')
		inputYReal2016.setAttribute("type", "text")
		inputYReal2016.setAttribute("id", `task-${this.taskNumber}-answer-yReal2016`)
		const inputYReal2017 = document.createElement('input')
		inputYReal2017.setAttribute("type", "text")
		inputYReal2017.setAttribute("id", `task-${this.taskNumber}-answer-yReal2017`)
		...
		<table>
			<tr>
				<td>Yn</td>
				<td>${inputYNom2016.outerHTML}%</td>
				<td>${inputYNom2017.outerHTML}%</td>
			</tr>
			<tr>
				<td>Yr</td>
				<td>${inputYReal2016.outerHTML}%</td>
				<td>${inputYReal2017.outerHTML}%</td>
			</tr>
			<tr>
				<td>Deflator</td>
	...
	private check() {
		const inputYNom2016 = document.getElementById(`task-${this.taskNumber}-answer-yNom2016`) as HTMLInputElement
		const inputYNom2017 = document.getElementById(`task-${this.taskNumber}-answer-yNom2017`) as HTMLInputElement
		const inputYReal2016 = document.getElementById(`task-${this.taskNumber}-answer-yReal2016`) as HTMLInputElement
		const inputYReal2017 = document.getElementById(`task-${this.taskNumber}-answer-yReal2017`) as HTMLInputElement
		...
		inputYNom2016.style.background = inputYNom2016.value == yNominal1 ? Colors.green : Colors.red
		inputYNom2017.style.background = inputYNom2017.value == yNominal2 ? Colors.green : Colors.red
		inputYReal2016.style.background = inputYReal2016.value == yReal1 ? Colors.green : Colors.red
		inputYReal2017.style.background = inputYReal2017.value == yReal2 ? Colors.green : Colors.red
